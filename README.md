...
| Technicians schedule the service call a claimed assistance request becomes; customer and technician both told, then given notice at a fixed hour (17:00) the evening before, whatever time the visit is | `views/support.php` (the drawer on a claimed request) → `Crm\Interventions` → `Crm\Automation::interventionReminders` (migration 063) |
...
| Every quote request lands in its own tab (Richieste offerta) with a count on the menu; open ones first, oldest first, and one press marks it handled | `views/offers.php` → `Inspections::offerRequests/openOfferCount/markOfferHandled` (migration 070) |
...
| The maintenance contract on the customer and lead record — subscription, gestionale contract, one typed in by the office, or **A CHIAMATA**; and a quarterly chase for the on-demand ones, counted from their last visit | `Crm\Maintenance` → the panel in `views/customers.php` / the pill in `views/leads.php`; the chase rides `bin/scheduler.php` → `public/support.php?vat_number=` (migration 066) |
