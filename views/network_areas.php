...
function naOpen(){ $('naModalTitle').textContent=NA.add; $('na_id').value=''; $('na_name').value=''; $('na_host').value='';
  ...
  $('na_count').value=2; $('na_sort').value=0; $('na_active').checked=true; $('naModalBg').classList.add('show'); }
function naEdit(a){ $('naModalTitle').textContent=NA.edit; $('na_id').value=a.id; $('na_name').value=a.name||'';
  ...
  $('na_sort').value=a.sort_order||0; $('na_active').checked=(a.active==1); $('naModalBg').classList.add('show'); }
...
function naSave(btn){
  ...
  fetch('device-api.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
    ...
    .then(function(j){ if(j&&j.ok){ location.reload(); } else { alert((j&&j.error)||'error'); btn.disabled=false; } })
